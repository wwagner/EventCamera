...
std::atomic<bool> frame_consumed{true};  // Track if previous frame was displayed
std::atomic<int64_t> last_frame_camera_ts{0};  // Camera timestamp (us since camera start)
std::atomic<int64_t> last_frame_system_ts{0};  // System timestamp when frame was generated
std::atomic<int64_t> last_display_time_us{0};  // Last time we actually displayed a frame
std::atomic<int64_t> last_event_camera_ts{0};  // Last event timestamp from camera
std::atomic<int64_t> frames_dropped{0};  // Count dropped frames
std::atomic<int64_t> frames_generated{0};  // Total frames generated by camera
std::atomic<int> target_display_fps{10};  // Target display rate (Hz)
std::atomic<int64_t> camera_start_time_us{0};  // System time when camera started
std::atomic<int64_t> total_events_received{0};  // Total events received from camera
std::atomic<int64_t> events_last_second{0};  // Events in the last second
std::atomic<int64_t> last_event_rate_update_us{0};  // Last time we updated event rate
...
void update_texture(const cv::Mat& frame) {
    ...
    if (!frame_consumed.load()) {
        return;  // Drop this frame - previous frame not yet displayed
    }

    std::lock_guard<std::mutex> lock(frame_mutex);
    current_frame = frame.clone();
    frame_consumed.store(false);  // Mark new frame as not yet consumed
}
...
void upload_frame_to_gpu() {
    ...
    // Mark frame as consumed - allows next frame to be stored
    frame_consumed.store(true);
}
...
bool try_connect_camera(AppConfig& config, std::map<std::string, BiasRange>& bias_ranges,
    ...
    camera_state.frame_gen->set_output_callback([](const Metavision::timestamp ts, cv::Mat& frame) {
        ...
        int64_t last_display = last_display_time_us.load();
        int fps_target = target_display_fps.load();
        int64_t frame_interval_us = 1000000 / fps_target;  // microseconds between frames

        // Only update if enough time has passed since last display
        if (now_us - last_display >= frame_interval_us) {
            // Check if previous frame was consumed
            if (frame_consumed.load()) {
                last_frame_camera_ts.store(ts);
                last_frame_system_ts.store(now_us);
                last_display_time_us.store(now_us);
                update_texture(frame);
            } else {
                // Display busy, drop this frame
                frames_dropped.fetch_add(1);
            }
        } else {
            // Rate limiting, drop this frame
            frames_dropped.fetch_add(1);
        }
    });
    ...
    camera_start_time_us.store(std::chrono::duration_cast<std::chrono::microseconds>(start_time.time_since_epoch()).count());
    ...
    camera_state.event_thread = std::make_unique<std::thread>([&]() {
        ...
        camera->cd().add_callback([&](const Metavision::EventCD* begin,
            ...
            total_events_received.fetch_add(event_count);

            // Track last event timestamp to measure event latency
            last_event_camera_ts.store((end-1)->t);

            // CRITICAL FIX: Skip ALL old event batches immediately
            // Check age of newest event - if old, skip entire batch without iterating
            auto now = std::chrono::steady_clock::now();
            auto now_us = std::chrono::duration_cast<std::chrono::microseconds>(now.time_since_epoch()).count();
            int64_t cam_start = camera_start_time_us.load();

            // Update event rate counter every second
            int64_t last_update = last_event_rate_update_us.load();
            if (last_update == 0) {
                // First time initialization
                last_event_rate_update_us.store(now_us);
            } else if (now_us - last_update >= 1000000) {  // 1 second passed
                // Calculate events per second
                int64_t total_now = total_events_received.load();
                int64_t elapsed_us = now_us - last_update;

                // Calculate rate: events per microsecond * 1,000,000 = events per second
                static int64_t last_total = 0;
                int64_t events_in_period = total_now - last_total;

                // Scale to exactly 1 second (handle slight timing variations)
                int64_t rate = (events_in_period * 1000000) / elapsed_us;
                events_last_second.store(rate);
...
int main(int argc, char* argv[]) {
    ...
    while (!glfwWindowShouldClose(window) && running) {
        ...
        if (ImGui::Begin("Camera Settings")) {
            ...
            if (camera_state.camera_connected && cam_info_ptr) {
                ...
                if (ImGui::Button("Disconnect & Reconnect Camera", ImVec2(-1, 0))) {
                    ...
                    if (camera_state.event_thread && camera_state.event_thread->joinable()) {
                        ...
                        camera_state.event_thread->join();
                        camera_state.event_thread.reset();
                        running = true;  // Re-enable for next connection
                        std::cout << "Event thread stopped" << std::endl;
                    }

                    // Clear camera resources
                    camera_state.frame_gen.reset();
                    camera_state.camera_mgr.reset();
                    camera_state.camera_connected = false;
            ...
            // Event rate display
            int64_t event_rate = events_last_second.load();
            ...
            int64_t gen = frames_generated.load();
            int64_t drop = frames_dropped.load();
            if (gen > 0) {
            ...
            int64_t event_ts = last_event_camera_ts.load();
            int64_t cam_start = camera_start_time_us.load();
            ...
            auto now = std::chrono::steady_clock::now();
            auto now_us = std::chrono::duration_cast<std::chrono::microseconds>(now.time_since_epoch()).count();
            int64_t frame_sys_ts = last_frame_system_ts.load();
            if (frame_sys_ts > 0) {
            ...
            int display_fps = target_display_fps.load();
            if (ImGui::SliderInt("Target Display FPS", &display_fps, 1, 60)) {
                target_display_fps.store(display_fps);
            }
        ...
        float camera_aspect = static_cast<float>(image_width) / image_height;
